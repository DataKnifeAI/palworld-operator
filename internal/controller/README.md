...
| `envoy_gateway.go` | Gateway, EnvoyProxy, UDPRoute, TCPRoute, HTTPRoute (mod manager) |
...
Saved mount (official): `/pal/Package/Pal/Saved`. Optional mods PVC (`spec.mods.enabled`): `/pal/Package/Mods` plus `Paks/~WorkshopMods` and `Paks/LogicMods` subpath overlays. Optional `spec.modManager`: sidecar file manager on Gateway HTTP port 8088.
